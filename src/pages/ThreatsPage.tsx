...
export default function ThreatsPage() {
  ...
  // Secondary sort: neutralised threats are moved to the bottom of the list,
  // and within that group ordered by resolvedDate (newest first).
  const sorted = [...filtered].sort((a, b) => {
    if (a.status === 'neutralised' && b.status !== 'neutralised') return 1
    if (a.status !== 'neutralised' && b.status === 'neutralised') return -1
    if (a.status === 'neutralised' && b.status === 'neutralised') {
      const numA = parseInt((a.resolvedDate ?? '').replace(/\D/g, ''), 10) || 0
      const numB = parseInt((b.resolvedDate ?? '').replace(/\D/g, ''), 10) || 0
      return numB - numA
    }
    return 0
  })

  // Style helpers — active threats get a red hover, neutralised ones get
  // a slightly greyed-out base and a green tint on hover (border only).
  const isNeutralised = (threat: (typeof filtered)[number]) => threat.status === 'neutralised'
  const baseBorder = (threat: (typeof filtered)[number]) =>
    isNeutralised(threat) ? '1px solid var(--graphite-800)' : '1px solid var(--graphite-700)'
  const baseBackground = 'rgba(30,30,34,0.5)'
  const hoverBorder = (threat: (typeof filtered)[number]) =>
    isNeutralised(threat) ? 'rgba(60,120,70,0.5)' : 'rgba(150,30,30,0.4)'
  const hoverBackground = 'rgba(30,30,34,0.8)'
        ...
        {sorted.map((threat) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'art-card group block rounded-lg p-6 transition-all',
                neutral && 'opacity-65',
              )}
              style={{
                border: baseBorder(threat),
                backgroundColor: baseBackground,
              }}
              onMouseEnter={e => {
                e.currentTarget.style.borderColor = hoverBorder(threat)
                e.currentTarget.style.backgroundColor = hoverBackground
              }}
              onMouseLeave={e => {
                e.currentTarget.style.borderColor = isNeutralised(threat) ? 'var(--graphite-800)' : 'var(--graphite-700)'
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <h2
                    className={cn(
                      'font-display text-xl font-semibold text-graphite-200 transition-colors',
                      neutral ? 'group-hover:text-emerald-400' : 'group-hover:text-red-300',
                    )}
                  >
                    {threat.name}
                  </h2>
                ...
                <ChevronRight
                  ...
                  className={cn(
                    ...
                    neutral ? 'text-emerald-900/60 group-hover:text-emerald-700' : 'text-red-900/60 group-hover:text-red-700',
                  )}
                />
              </div>
            </Link>
